...
export default function ClientFacturation({ 
  ...
  const handlePrint = () => {
    ...
    const itemsRows = items.map(item => `<tr><td>${item.concepto}</td><td class="num">${item.cantidad}</td><td class="num">${item.precio_unitario.toFixed(2)}</td><td class="num">${item.iva_porc}%</td><td class="num">${(item.cantidad * item.precio_unitario).toFixed(2)} EUR</td></tr>`).join('');
    const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Factura '+nFactura+'</title><style>body{font-family:"Segoe UI",Arial,sans-serif;font-size:10pt;color:#1e293b;padding:30px 40px;max-width:210mm;margin:0 auto}.header{display:flex;justify-content:space-between;border-bottom:2px solid #1e293b;padding-bottom:10px;margin-bottom:16px}.header-r{text-align:right;font-size:8pt}.company{font-size:7pt;color:#64748b}h1{font-size:16pt;margin:0}.info-table{width:100%;margin:12px 0;font-size:9pt}.info-table td{padding:3px 8px;border:1px solid #e2e8f0}.info-table .lbl{background:#f8fafc;font-weight:600;width:35%}table.items{width:100%;border-collapse:collapse;margin:16px 0;font-size:9pt}table.items th{background:#f1f5f9;padding:6px 8px;border:1px solid #cbd5e1;text-align:left;font-size:8pt;text-transform:uppercase}table.items td{padding:5px 8px;border:1px solid #e2e8f0}.num{text-align:right;font-family:Consolas,monospace}.total-table{width:100%;border-collapse:collapse;margin-top:12px}.total-table td{padding:5px 10px;border:1px solid #e2e8f0;font-size:9pt}.total-table .grand td{font-weight:800;font-size:11pt;border-top:2px solid #1e293b;background:#f8fafc}.signature{margin-top:50px;display:flex;justify-content:space-between}.sig-line{border-top:1px solid #1e293b;width:200px;margin-top:40px}.sig-label{font-size:7pt;color:#64748b;margin-top:4px}.footer{margin-top:25px;font-size:7pt;color:#94a3b8;text-align:center;border-top:1px solid #e2e8f0;padding-top:8px}@media print{body{padding:0}}</style></head><body><div class="header"><div><h1>FACTURA</h1><div class="company"><strong>'+hi.company+'</strong> | CIF: '+hi.cif+'<br>'+hi.address+'<br>'+hi.email+' | '+hi.tel+'</div></div><div class="header-r"><strong>N Factura:</strong> '+nFactura+'<br><strong>Fecha:</strong> '+fecha+'</div></div><table class="info-table"><tr><td class="lbl">Cliente</td><td><strong>'+clienteNombre+'</strong></td></tr><tr><td class="lbl">NIF / CIF</td><td>'+clienteNif+'</td></tr><tr><td class="lbl">Direccion</td><td>'+(clienteDireccion||'')+'</td></tr><tr><td class="lbl">Email</td><td>'+(clienteEmail||'')+'</td></tr><tr><td class="lbl">Metodo de Pago</td><td>'+metodoPago+'</td></tr></table><table class="items"><thead><tr><th>Concepto</th><th class="num">Cant.</th><th class="num">Precio Ud.</th><th class="num">IVA</th><th class="num">Subtotal</th></tr></thead><tbody>'+itemsRows+'</tbody></table><table class="total-table"><tr><td style="width:60%"></td><td class="lbl">Base Imponible</td><td class="num">'+calculatedClientInvoice.totalBase.toFixed(2)+' EUR</td></tr><tr><td></td><td class="lbl">IVA</td><td class="num">'+calculatedClientInvoice.totalIva.toFixed(2)+' EUR</td></tr><tr class="grand"><td></td><td><strong>TOTAL FACTURA</strong></td><td class="num"><strong>'+calculatedClientInvoice.totalFactura.toFixed(2)+' EUR</strong></td></tr></table>'+(notas?('<p style="font-size:8pt;color:#64748b;margin-top:12px"><strong>Notas:</strong> '+notas+'</p>'):'')+'<div class="signature"><div><div class="sig-line"></div><div class="sig-label">Firma del Emisor</div></div><div><div class="sig-line"></div><div class="sig-label">Firma del Receptor</div></div></div><div class="footer">'+hi.company+' | '+hi.cif+' | '+hi.address+'</div><script>window.onload=function(){setTimeout(function(){window.print();},300);}<\/script></body></html>';
    const blob = new Blob([html], { type: 'text/html' });
    const url = URL.createObjectURL(blob);
    const w = window.open(url, '_blank');
    if (w) { w.onload = function() { URL.revokeObjectURL(url); w.print(); }; setTimeout(function() { try { if(w.closed) URL.revokeObjectURL(url); } catch(e){} }, 10000); }
