...
export default function ClientFacturation({ 
  ...
  const handlePrint = () => {
    ...
    const itemsRows = (inv.items || []).map((item: any) => {
      const sub = (item.cantidad || 1) * (item.precio_unitario || 0);
      const iva = sub * ((item.iva_porc || 10) / 100);
      return `<tr><td>${item.concepto}</td><td class="num">${item.cantidad || 1}</td><td class="num">${(item.precio_unitario || 0).toFixed(2)}</td><td class="num">${item.iva_porc || 10}%</td><td class="num">${sub.toFixed(2)} €</td></tr>`;
    }).join('');
    const w = window.open('', '_blank', 'width=800,height=700');
    if (!w) return;
    w.document.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Factura '+inv.n_factura+'</title><style>body{font-family:Segoe UI,Arial,sans-serif;font-size:10pt;color:#1e293b;padding:30px 40px;max-width:210mm;margin:0 auto}.header{display:flex;justify-content:space-between;border-bottom:2px solid #1e293b;padding-bottom:10px;margin-bottom:16px}.header-r{text-align:right;font-size:8pt}.company{font-size:7pt;color:#64748b}h1{font-size:16pt;margin:0}.info-table{width:100%;margin:12px 0;font-size:9pt}.info-table td{padding:3px 8px;border:1px solid #e2e8f0}.info-table .label{background:#f8fafc;font-weight:600;width:35%}table.items{width:100%;border-collapse:collapse;margin:16px 0;font-size:9pt}table.items th{background:#f1f5f9;padding:6px 8px;border:1px solid #cbd5e1;text-align:left;font-size:8pt;text-transform:uppercase}table.items td{padding:5px 8px;border:1px solid #e2e8f0}.num{text-align:right;font-family:Consolas,monospace}.total-table{width:100%;border-collapse:collapse;margin-top:12px}.total-table td{padding:5px 10px;border:1px solid #e2e8f0;font-size:9pt}.total-table .grand td{font-weight:800;font-size:11pt;border-top:2px solid #1e293b;background:#f8fafc}.signature{margin-top:50px;display:flex;justify-content:space-between}.sig-line{border-top:1px solid #1e293b;width:200px;margin-top:40px}.sig-label{font-size:7pt;color:#64748b;margin-top:4px}.footer{margin-top:25px;font-size:7pt;color:#94a3b8;text-align:center;border-top:1px solid #e2e8f0;padding-top:8px}@media print{body{padding:0}}</style></head><body><div class="header"><div><h1>FACTURA</h1><div class="company"><strong>'+hi.company+'</strong> | CIF: '+hi.cif+'<br>'+hi.address+'<br>'+hi.email+' | '+hi.tel+'</div></div><div class="header-r"><strong>Nº Factura:</strong> '+inv.n_factura+'<br><strong>Fecha:</strong> '+inv.fecha+'</div></div><table class="info-table"><tr><td class="label">Cliente</td><td><strong>'+inv.cliente_nombre+'</strong></td></tr><tr><td class="label">NIF / CIF</td><td>'+inv.cliente_nif+'</td></tr><tr><td class="label">Direccion</td><td>'+(inv.cliente_direccion||'')+'</td></tr><tr><td class="label">Email</td><td>'+(inv.cliente_email||'')+'</td></tr><tr><td class="label">Metodo de Pago</td><td>'+inv.metodo_pago+'</td></tr></table><table class="items"><thead><tr><th>Concepto</th><th class="num">Cant.</th><th class="num">Precio Ud.</th><th class="num">IVA</th><th class="num">Subtotal</th></tr></thead><tbody>'+itemsRows+'</tbody></table><table class="total-table"><tr><td style="width:60%"></td><td class="label">Base Imponible</td><td class="num">'+inv.total_base.toFixed(2)+' €</td></tr><tr><td></td><td class="label">IVA</td><td class="num">'+inv.total_iva.toFixed(2)+' €</td></tr><tr class="grand"><td></td><td><strong>TOTAL FACTURA</strong></td><td class="num"><strong>'+inv.total_factura.toFixed(2)+' €</strong></td></tr></table>'+(inv.notas?('<p style="font-size:8pt;color:#64748b;margin-top:12px"><strong>Notas:</strong> '+inv.notas+'</p>'):'')+'<div class="signature"><div><div class="sig-line"></div><div class="sig-label">Firma del Emisor</div></div><div><div class="sig-line"></div><div class="sig-label">Firma del Receptor</div></div></div><div class="footer">'+hi.company+' | '+hi.cif+' | '+hi.address+' | Registrado por: '+(inv.usuario_creador||'')+'</div></body></html>');
    w.document.close();
